...
use MakinaCorpus\Layout\Render\HtmlInjectionTrait;

/**
 * Decorates another rendererd, and injects edit links when necessary.
 */
class FooEditRendererDecorator implements GridRendererInterface
{
    ...
    public function renderItem(ItemInterface $item, ContainerInterface $parent, string $innerHtml, int $position, array $attributes = []) : string
    {
        ...
        $addition = ''; //$this->renderMenu($item, $this->getItemButtons($item, $parent, $position));
...
EOT;
    ...
    private function renderLink(string $title, string $route, array $parameters, string $icon = null, bool $disabled = false) : string
    {
        ...
        if ($icon) {
            $title = '<span class="fa fa-' . $icon . '" aria-hidden="true"></span> ' . $title;
        }
